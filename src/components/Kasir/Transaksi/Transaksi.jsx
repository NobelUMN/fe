...
function Transaksi({ onLogout, setActiveMenu, activeMenu, authUserName }) {
  ...
  const addToCart = useCallback((item) => {
    setCart(prev => {
      ...
      // Cari produk berdasarkan barcode (3-tier: local -> backend list -> barcode endpoint)
  const processBarcode = useCallback(async (code) => {
    ...
    // Tier 1: Search local produk state
    let local = produkRef.current.find(p => [p.id_barcode, p.barcode, p.kode_barcode, p.id_produk, p.id]
      .map(x => String(x ?? ''))
      .includes(trimmed));
    if (local) {
      addToCart(local);
    ...
    try {
      ...
      if (listRes.ok) {
        ...
        local = normalizedList.find(p => [p.id_barcode, p.barcode, p.kode_barcode, p.id_produk, p.id]
          .map(x => String(x ?? ''))
          .includes(trimmed));
        if (local) {
          addToCart(local);
    ...
    try {
      ...
      const response = await fetch('https://be-production-6856.up.railway.app/api/transaksi', {
        ...
        headers: { 
          ...
          'Authorization': `Bearer ${token}`  // ✅ TAMBAH AUTHORIZATION TOKEN
        },
        body: JSON.stringify({
          tanggal,
